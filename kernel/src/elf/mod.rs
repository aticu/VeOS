...
pub fn debug() {
    let mut file = ElfFile::from_initramfs("/bin/init").unwrap();
    ...
    ::multitasking::READY_LIST
        .lock()
        .push(::multitasking::TCB::elf_test(32768,
                                            file.header.program_entry as u64,
                                            &mut address_space));

    // let kernel_stack_size = 0x5000;
    // let kernel_stack_start = 0xfffffe2000400000 - kernel_stack_size;

    // address_space.add_segment(Segment::new(kernel_stack_start,
    // kernel_stack_size, ::memory::WRITABLE | ::memory::READABLE));

    // let stack_data = unsafe { ::core::slice::from_raw_parts(kernel_stack_start
    // as *const u8, kernel_stack_size) };

    // address_space.write_to(stack_data, kernel_stack_start);

    // for i in 0..header.program_header_entry_num as usize {

    // :memory::map_range(program_header.virtual_address,
    // program_header.memory_size, ::memory::READABLE | ::memory::WRITABLE |
    // ::memory::EXECUTABLE | ::memory::USER_ACCESSIBLE);

    // let src: &[u8] = unsafe { ::core::slice::from_raw_parts((fd.start +
    // program_header.offset) as *const u8, program_header.file_size) };

    // let dst: &mut [u8] = unsafe {
    // ::core::slice::from_raw_parts_mut(program_header.virtual_address as *mut u8,
    // program_header.memory_size) };

    // for i in 0..program_header.file_size {
    // dst[i] = src[i];
    //


    // :multitasking::READY_LIST.lock().push(::multitasking::TCB::elf_test(32768,
    // header.program_entry as u64));
}
